...
int main(){
	...
	string resultsfolder_og = "Data/Manypoints_IntHill_consistent_violator_largeN/" +return_current_time_and_date();
    ...
    long int Nmax = 1e10;
	long int Nsteps = 5e7;
    std::uniform_int_distribution<int> initial_state_dist(1,50); // To start each simulation independently
    ...
    std::vector<double> d_b_vals = {0.05};
    ...
    for(double da:d_a_vals){
        for(double db:d_b_vals){
                ...
                itr1++;
                std::vector<std::vector<double>> pertlist = {{1.,1+da,1-da,1+0.1*da,1-0.1*da,1+0.5*da,1-0.5*da,
                                                              1+5*da,1-5*da,1+10*da,1-10*da},{1.},
                                                              {1.,1+db,1-db,1+0.1*db,1-0.1*db,1+0.5*db,1-0.5*db,
                                                                1+5*db,1-5*db,1+10*db,1-10*db},{1.},
                                                            {1.},{1.},
                                                            {1.},{1.},
                                                            {1.},{1.},
                                                            {1.}};
                ...
                #pragma omp parallel for shared(allres) num_threads(1) schedule(dynamic,1)
                {
                    for(int resrow = 0;resrow < perts.size();resrow++){
                        ...
                        for(int i=0; i < pert.size(); i++){
                            ...
                            xyz_intyhill system{params.at(0),params.at(1),params.at(2),params.at(3),
                                                    params.at(4),params.at(5),params.at(6),params.at(7),params.at(8),params.at(9),params.at(10)};
                            ...
                            covbaldata_3D data = alldata.cbvals;
                            double w = data.wsum;
                            double zero = 0;

                            /*saving results {"Exec Time","Sim Time","Steps",
                                            "Flux Bal x","Flux Bal y","Flux Bal z",
                                            "Cov Bal xx","Cov Bal xy","Cov Bal yy",
                                            "Cov Bal xz","Cov Bal yz","Cov Bal zz",
                                            "lambda","beta_x","gamma","beta_y","alpha","beta_z",
