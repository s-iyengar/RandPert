...
int main(){
	...
	string resultsfolder_og = "Data/NegFy1/" +return_current_time_and_date();
    ...
    std::vector<double> d_a_vals = log_space(-2,-0.01,10,15);
    std::vector<double> d_b_vals = log_space(-1,-0.01,10,15);
                ...
                #pragma omp parallel for shared(allres) num_threads(3) schedule(dynamic,1)
                {
                    for(int resrow = 0;resrow < perts.size();resrow++){
                        ...
                        for(int i=0; i < pert.size(); i++){
                            ...
                            if((params.at(4)==0)&(params.at(6)==0)){params.at(0) /= params.at(8);}
