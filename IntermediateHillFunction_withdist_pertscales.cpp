...
int main(){
	...
	string resultsfolder_og = "Data/singlelineplots/" +return_current_time_and_date();
    ...
    std::vector<double> pertscales = {0.01};


    
    std::vector<std::vector<double>> dnames(pertscales.size()*3,std::vector<double>(3));
    int itr1 = 0;
    //sqrt(N)=db/da
    for(double pertscale:pertscales){
        std::vector<double> d_a_vals = {0};
        std::vector<double> d_b_vals = {0.5*pertscale,pertscale,1.5*pertscale};
        for(double da:d_a_vals){
            for(double db:d_b_vals){
                    ...
                    itr1++;
                    std::vector<std::vector<double>> pertlist = {{1.},{1.},{1.,1+db,1-db},{1.},
                                                                {1.},{1.},
                                                                {1.},{1.},
                                                                {1.}};
        ...
        #pragma omp parallel for num_threads(1) schedule(dynamic, 1)
        {
            for(int basenum = 0;basenum < baseparamlist.size();basenum++){
                    ...
                    for(int pertnum = 0;pertnum < bigpertlist.size();pertnum++){
                        ...
                        #pragma omp parallel for shared(allres) num_threads(4) schedule(dynamic,1)
                        {
