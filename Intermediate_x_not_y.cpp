...
struct covbal_directy_indirectx{
    covbaldata_3D cb;
    ...
    double meanF_y;
    double meanF_x;
    double lambda,nx,Kx,ny,Ky,alpha,beta_z;
    covbal_directy_indirectx() = default;
    covbal_directy_indirectx(std::vector<double> pars){
        params = pars;
        cb = covbaldata_3D();
        meanF_y = 0.0;
        meanF_x = 0.0;
    ...
    void recordData(const state_t &currentstate,const rate_list &rates,const double &w){
        cb.recordData(currentstate,rates,w);
        double rw = w / cb.wsum;
        ...
        double Rxp = rates[0];
        double Rzp = rates[4];
        //fx is equivalent to dlog(Rz+)/dlog(x)*dlog(Rx+)/dlog(z)
        //fy is equivalent to dlog(Rx+)/dlog(y)
        double fx = ((lambda*(double)z)/Rxp)*x_sense_calc(x,nx,Kx);
        double fy = (lambda/Rxp)*alpha*sense_calc(y,ny,Ky);
    ...
    double sense_calc(int x,double n, double K){
        if(x == 0) return 0;
        else{
            double dx = (double)x;
            return (n*pow(dx,n)*pow(K,n))/((pow(dx,n)+pow(K,n))*(pow(dx,n)+pow(K,n)));
        }
    }
    double x_sense_calc(int x,double n,double K){
        double dx = (double)x;
        return n*pow(K,n)/(pow(dx,n)+pow(K,n));
    }
};
struct covbal_withpdist{
    covbal_directy_indirectx cbvals;
    std::vector<double> params; 
    vector_map_unordered pdist;
    covbal_withpdist(std::vector<double> pars){
        params = pars;
        cbvals = covbal_directy_indirectx{params};
...
struct xyz_directyfb{
	 ...
	 void updateRates(state_t const &state,rate_list &rates){
		 ...
		 int z = state[2];
		 //calculate rates
		 rates[0] = lambda*(z+alpha*hill_function_pure(y,Ky,ny)+offset);
		 rates[1] = beta_x*x;
		 rates[2] = gamma*x;
		 rates[3] = beta_y*y;
		 rates[4] = hill_function_pure(x,Kx,nx);
		 rates[5] = beta_z*z;
	 }
...
int main(){
	...
	string resultsfolder_og = "Data/Directy_largeNspread/" +return_current_time_and_date();
    ...
    std::vector<double> d_b_vals = {0.05158};
                    ...
                    #pragma omp parallel for shared(allres) num_threads(3) schedule(dynamic,1)
                    {
                        for(int resrow = 0;resrow < perts.size();resrow++){
                                ...
                                for(int i=0; i < pert.size(); i++){
                                    ...
                                    xyz_directyfb system{params[0],params[1],params[2],params[3],
                                    ...
                                    covbaldata_3D data = alldata.cbvals.cb;
                                    double w = data.wsum;
                                    double zero = 0;
