...
int main(){
	...
	string resultsfolder_og = "Data/weirdfb_varnx/" +return_current_time_and_date();
	...
	//{lambda,beta_x,gamma,beta_y,nx,K,offset}

    std::vector<std::vector<double>> paramgen = {{1.},{0.4},{5.},{1.},{2.},
                                                            {10.},{0.1}};
    ...
    std::vector<double> d_b_vals = log_space(-1,-0.3,10,5);
                ...
                #pragma omp parallel for shared(allres) num_threads(3) schedule(dynamic,1)
                {
                    for(int resrow = 0;resrow < perts.size();resrow++){
                        ...
                        for(int i=0; i < pert.size(); i++){
                            ...
                            weird_bothfb_xy system{params.at(0),params.at(1),params.at(2),params.at(3),
                            ...
                            std::vector<double> streaming_data = {exec_time,w,dsteps,
                                                        ...
                                                        system.lambda,system.beta_x,system.gamma,system.beta_y,system.K,system.nx,system.offset,
